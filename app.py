    ...
    if st.button("🚀 Gerar roteiro inteligente") or pesquisa:
        ...
        if not lista: st.warning("🤖 Ops! Não encontrei nada.")
            ...
            for item in lista:
                ...
                st.markdown(f"### 📍 {nome_atrativo} ({score})")
                st.info(f"💡 {item['dica']}")
...
with tab3:
    ...
    st.write("O X.Tur é um sistema especialista focado em análise, triagem e recomendação de roteiros baseada em dados reais e verificáveis para transformar a complexidade logística do turismo em uma experiência fluida.")
