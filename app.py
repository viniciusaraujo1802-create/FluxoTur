...
def injetar_vlibras():
    ...
    <script>window.onload = function() { new window.VLibras.Widget('https://vlibras.gov.br/app'); }</script>
    """
    ...
    if btn:
        ...
        for nome, item in atrativos_db.items():
            if not categoria_input or categoria_input.strip().lower() in item["cat"].lower():
                ...
                # Metodologia: Reputação (peso 2) + Conforto de trânsito + Conforto de capacidade
        ...
        for r in resultados:
            ...
            st.write(f"**Reputação Digital:** {r['reputacao']} | **Trânsito:** {r['transito']} | **Capacidade:** {r['capacidade']}")
...
with tab3:
    ...
    st.write("Metodologia de Ranking: O score é calculado como (Reputação × 2) + Peso de Trânsito + Peso de Capacidade, garantindo que locais bem avaliados e com melhor fluxo tenham prioridade.")
